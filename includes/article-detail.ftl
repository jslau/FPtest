...
<#attempt>
	<#assign returnUrl=page.params.url/>
<#recover>
	<#assign returnUrl="http://${site.domain}/${page.name}?id=${page.params.id}"/>
</#attempt>
...
<#list feed.rss.channel.item as item> 
	...
	<#if item.guid == page.params.id>
		...
		<#-- Share links -->
<#--
		<@p.shareLinks returnUrl item.title "share"/>
-->
		<@p.shareLinks2 returnUrl item.link item.title "share"/>
		<@p.grayline/>
	<#else>
