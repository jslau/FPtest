...
Scroller.prototype.onTouchMove = function(e) {
  ...
  if(!this.ignoreTouch){
    ...
    //need prevent default because of bug in android.
    var delta = this.current[this.axis] - this.startTouch[this.axis];
    if(delta > 10 || delta < -10){
        console.log('move prevent default'+delta);
        e.preventDefault();
    }
...
Scroller.prototype.onTouchEnd = function(e) {
    ...
    if(!this.ignoreTouch){
        ...
        console.log('ontouch end not ignored');
        this.ignoreTouch = true;
...
Scroller.prototype.openArticle = function(e){
  ...
  for (var i=0; i < cl; i++){
          ...
          if (c[i].hasClass("arrow") || c[i].hasClass("undersideLink")) {
            //check to see if the href == #, then call onclick() for the anchor
              if ( c[i].href == location.href+'#' ) {
                  c[i].onclick();
                  break;
              } else {
...
Scroller.prototype.overElm = function(finger){
    ...
    if(!this.elm.getBoundingClientRect && ua=='iphone'){
    ...
    if(window.scrollviewer == null){
        ...
        if (finger[1] < bounds.bottom &&
           finger[1] > bounds.top &&
           finger[0] > bounds.left &&
           finger[0] < bounds.right){
          //console.log(this.elm)
         // alert("overelm")
          return true;
        }
...
Scroller.prototype.animateTo = function(y) {
  ...
  offset=y;
  if(this.axis == 0){
    translate = 'translate3d(' + y + 'px, 0, 0)';

    //this.elm.style['MozTransform'] = translate;
    //this.elm.style['OTransform'] = translate;
    //this.elm.style['msTransform'] = translate;
    this.elm.style.webkitTransform = translate;
    //this.elm.style['transform'] = 'translate(' + y + 'px)';
...
function promptHomeScreen(){
  var phsElm = document.createElement('div');
  var close = document.createElement('div');
  close.id = 'closeModal';
  close.innerHTML = 'x';
  phsElm.id = "promptHomeScreen";
  phsElm.innerHTML =  '<div class="logo"></div><p>Install this web app on your phone: tap on the arrow and then <b>\'Add to Home Screen.\'</b></p><div class="arrow-down"></div><div class="arrow-down" style="  border-top: 20px solid #c2c2c2; bottom: -17px;"></div>';

  phsElm.appendChild(close);
  ...
  phsElm.style.opacity = 1;
  phsElm.style.bottom = "20px";
  $("body").appendChild(phsElm);
